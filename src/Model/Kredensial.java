package Model;

import classes.Database;
import java.util.ArrayList;
import java.util.HashMap;

public class Kredensial {
    
    private static final Database db = new Database();
    
    public static ArrayList<HashMap<String, String>> Get() {
        var result = db.Table("kredensial")
                       .Select("id", "username", "password", "level")
                       .Get();
        return result;
    }
    
    public static ArrayList<HashMap<String, String>> Get(int id) {
        var condition = new HashMap<String, String>() {{
            put("id", String.valueOf(id));
        }};
        var result = db.Table("kredensial")
                       .Select("id", "username", "password", "level")
                       .Where(condition)
                       .Get();
        return result;
    }
    
    public static ArrayList<HashMap<String, String>> Get(String username) {
        var condition = new HashMap<String, String>() {{
            put("username", username);
        }};
        var result = db.Table("kredensial")
                       .Select("id", "username", "password", "level")
                       .Where(condition)
                       .Get();
        return result;
    }
    
    public static void Add(String ...data) {
        String[] fields = {"username", "password", "level"};
        var buffer = new HashMap<String, String>();
        for (int i = 0; i < fields.length; i++) {
            buffer.put(fields[i], data[i]);
        }
        db.Table("kredensial").Insert(buffer);
    }
    
    public static void Edit(int id, String ...data) {
        String[] fields = {"username", "password", "level"};
        var buffer = new HashMap<String, String>();
        for (int i = 0; i < fields.length; i++) {
            buffer.put(fields[i], data[i]);
        }
        var condition = new HashMap<String, String>() {{
            put("id", String.valueOf(id));
        }};
        db.Table("kredensial").Where(condition).Update(buffer);
    }
    
    public static void Destroy(int id) {
        var condition = new HashMap<String, String>() {{
            put("id", String.valueOf(id));
        }};
        db.Table("kredensial").Where(condition).Delete();
    }
    
}
